...
void parse_l2book_data(auto &result, auto &value, auto &buffer) {
  enum class Key {
    COIN,
    TIME,
    LEVELS,
    ...
    switch (key_2) {
      ...
      case Key::LEVELS: {
        ...
        for (auto value_2 : std::get<core::json::Array>(value)) {
          switch (++offset) {
            case 1:
              result.bids = core::json::ArrayParser<decltype(result.bids), core::json::Array>::parse(buffer, std::get<core::json::Array>(value_2));
              break;
            case 2:
              result.asks = core::json::ArrayParser<decltype(result.asks), core::json::Array>::parse(buffer, std::get<core::json::Array>(value_2));
              break;
            default:
...
auto parse_l2book(auto &value, auto &buffer) {
  L2Book result;
  enum class Key {
    CHANNEL,
    DATA,
  ...
  for (auto [key, value] : std::get<core::json::Object>(value)) {
    ...
    switch (key_2) {
      case Key::CHANNEL:
        ...
        break;
      case Key::DATA:
        parse_l2book_data(result.data, value, buffer);
        break;
    }
  }
  return result;
...
void dispatch_l2book(auto &handler, auto &message, auto &buffer, auto &trace_info) {
  core::json::Buffer buffer_2{buffer};
  core::json::Parser parser{message};
  auto value = parser.root();
  auto obj = parse_l2book(value, buffer_2);
  create_trace_and_dispatch(handler, trace_info, obj);
}
}  // namespace
...
bool Parser::dispatch(Handler &handler, std::string_view const &message, std::span<std::byte> const &buffer, TraceInfo const &trace_info) {
  ...
  switch (message_.channel) {
    ...
    case BBO:
      ...
      return true;
    case L2_BOOK:
      dispatch_l2book(handler, message, buffer, trace_info);
      return true;
    case TRADES:
