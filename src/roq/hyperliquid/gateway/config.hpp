...
struct ROQ_PUBLIC Config final : public server::config::Dispatcher, public server::config::Reader::Handler {
 ...
 private:
  ...
  std::span<std::string const> const dex_;
...
template <>
struct fmt::formatter<roq::hyperliquid::gateway::Config> {
  constexpr auto parse(format_parse_context &context) { return std::begin(context); }
  auto format(roq::hyperliquid::gateway::Config const &value, format_context &context) const {
